   ...
   todo.find({},(err,todolist)=>{
    action.find({}, (err,actionlist)=>{
      apprec.find({}, (err,appreclist)=>{
        res.render('index.ejs',{todolist:todolist,actionlist:actionlist,appreclist:appreclist});
    });    
    });
  });
...
app.post('/update/:id', (req,res) =>{
  ...
  todo.findByIdAndUpdate(id, {$set:{name : body}}, {new:true}).then((todo)=>{
       
       res.redirect('/');
  });

  action.findByIdAndUpdate(id, {$set:{name : body1}}, {new:true}).then((action)=>{
    
    res.redirect('/');
});

apprec.findByIdAndUpdate(id, {$set:{name : body2}}, {new:true}).then((apprec)=>{
